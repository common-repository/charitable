...
if ( ! class_exists( 'Charitable_Campaign_Processor' ) ) :
	...
	class Charitable_Campaign_Processor {
		 ...
		 *
		 * @since  1.5.9
		 * @since  1.8.1 Added two action hooks.
		 *
		...
		public function save_core() {
			...
			if ( $data['ID'] ) {
				...
				// The returned value can be either the campaign_id or 0 on fail.
				$campaign_id = wp_update_post( $data );

				/**
				 * Peform an action after the campaign has been updated.
				 *
				 * @since 1.8.1
				 *
				 * @param array                                     $data      Core data.
				 * @param Charitable_Campaign_Processor $processor This processor object.
				 */
				do_action( 'charitable_campaign_processor_save_core', 'update', $campaign_id, $data, $this );

				return $campaign_id;
			}

			// The returned value can be either the campaign_id or 0 on fail.
			$campaign_id = wp_insert_post( $data );
			 ...
			 * @param Charitable_Campaign_Processor $processor This processor object.
			 */
			do_action( 'charitable_campaign_processor_save_core', 'insert', $campaign_id, $data, $this );
