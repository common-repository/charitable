...
var CharitableChecklist =
  window.CharitableChecklist ||
    ...
    var app = {
      ...
      setup: function () {
        ...
        if (
          ...
          typeof charitable_admin_checklist_onboarding === "undefined" ||
          typeof charitable_admin_checklist_onboarding.option === "undefined" ||
          typeof charitable_admin_checklist_onboarding.option.tour ===
            "undefined" ||
          typeof charitable_admin_checklist_onboarding.option.tour.status ===
            "undefined"
        ) {
          // eslint-disable-line
          return;
        ...
        if (
          window.location.search.includes("checklist") &&
        ...
        if (
          window.location.search.includes("page=charitable-setup-checklist")
        ) {
          app.setupChecklistPage();
        }
        ...
        if (
          charitable_admin_checklist_onboarding.option.tour.status === "init" ||
          charitable_admin_checklist_onboarding.option.tour.status ===
            "started" ||
          charitable_admin_checklist_onboarding.option.tour.status === ""
        ) {
          ...
          charitable_admin_checklist_onboarding.option.tour.status === "started"
        ) {
        }
      },
      ...
      setupChecklistPage: function () {
        ...
        $(".charitable-step.charitable-step-fundraising-next-level").on(
            "click",
            ".charitable-button",
            function () {
                ...
                updateStatus.done(function() {
                    // reload the page without any hash.
        ...
        if (window.location.hash === "#general-settings") {
          ...
          $step_element = $(
            ".charitable-step.charitable-step-create-first-campaign",
          );
        } else if (window.location.hash === "#first-donation") {
          $step_element = $(
            ".charitable-step.charitable-step-create-first-donation",
          );
        } else if (window.location.hash === "#next-level") {
          $step_element = $(
            ".charitable-step.charitable-step-fundraising-next-level",
          );
        } else {
          return;
        }
       ...
       * @since 1.8.1.15
       */
      setupChecklistGeneralSettings: function () {
        // if the url contains the query name of 'checklist' and the value of 'general-settings', then start the tour.
        ...
        if (
          charitable_admin_checklist_onboarding.option.tour.steps_completed.includes(
            "general-settings",
          )
        ) {
          return;
        ...
        checklistTour.addStep({
          ...
          text: charitable_admin_checklist_onboarding.strings
            .general_settings_step_0_text,
          arrow: false,
       ...
       * @since 1.8.1.15
       */
      setupChecklistEmailSettings: function () {
        // if the url contains the query name of 'checklist' and the value of 'email-settings', then start the tour.
       ...
       * @since 1.8.1.15
       */
      setupChecklistGatewaySettings: function () {
        // if the url contains the query name of 'checklist' and the value of 'connect-gateway', then start the tour.
      ...
      events: function () {
        ...
        $(".charitable-checklist-list-block").on(
            "click",
            ".checklist-skip",
            function ( e ) {
                app.skipChecklist( e );
